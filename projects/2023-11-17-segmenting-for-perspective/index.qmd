...
ggplot(dept_comp_n, aes(x= annual_interval, y= dept, fill = Count)) +
  ...
  coord_fixed()+
  labs(title = 'The R&D department has the largest number of employees who earn between 150k-200k. \n37% of the R&D department, 305 employees, fall into this salary range. \nThe Sales department has the same percentage of employees earning between 150k-200k, \nbut due to its smaller size, this percentage represents only 130 employees.', 
       x= 'Compensation', 
       y= 'Department') +
...
ggplot(employees1, aes(x=annual_comp, y=job_title, fill=dept))+
  ...
  guides(fill="none")+
  labs(title = "The Sales Manager role has the largest average (median) compensation ($164.320), \nfollowed by the HR Manager ($151.840) for the manager role. \nThe Vice President role has the smallest average compensation ($68.640). \n",
       x= "Compensation",
       y= "Role")+
...
ggplot(employees1, aes( x= job_level, y=annual_comp, fill=dept))+
  ...
  theme(
    ...
    strip.text = element_textbox(size=10,color = "#404040", fill = "#A8DAFF" , box.color = "#A8DAFF" ,face = "bold", halign = 0.5, linetype = 1, r= unit(5, "pt"), width = unit(1, "npc"),
      ...
      panel.spacing = unit(1, "lines"),
      axis.text.x = element_text(size = 7))+
    labs(title = "Senior level of experience in the Sales department has the largest \naverage (median) compensation. The Expert level* of experience in the HR department \nhas the smallest. \n",
       y= "Compensation",
       x= "Experience Levels")+
  coord_flip()+
  guides(fill="none")
...
ggplot(comp_gen_dept, aes( x= generation, y=annual_comp, fill=dept))+
  ...
  scale_y_continuous(labels = scales::label_number(scale = 1e-3, suffix = "k", accuracy = 0.1),
                     limits= c(50e3, 250e3), breaks = seq(50e3, 250e3, by=50e3))+
  labs(title = "The Baby Boomers group from the HR has the smallest average (median) compensation,\n followed by the Generation Z group from the Sales department.\n",
       y= "Compensation",
       x= "Generation")+
  coord_flip()+
  guides(fill="none")
...
ggplot(comp_gender_dept, aes( x= gender, y=annual_comp, fill=dept))+
  ...
  scale_y_continuous(labels = scales::label_number(scale = 1e-3, suffix = "k", accuracy = 0.1),
                     limits= c(50e3, 250e3), breaks = seq(50e3, 250e3, by=50e3))+
  labs(title = "Women from the HR department have the smallest average (median) compensation \ncompared with men within the same department and women from the other \ndepartments.\n",
       y= "Compensation",
       x= "Gender")+
  coord_flip()+
  guides(fill="none")
...
-   The average (median) annual compensation across all departments is $137.180
...
-   Compensation by Department
    ...
    -   The R&D department has the largest number of employees who earn between 150k-200k. 37% of the R&D department, 305 employees, fall into this salary range.      -   The Sales department has the same percentage of employees earning between 150k-200k, but due to its smaller size, this percentage represents only 130 employees.

-   Compensation by Role

    -   A significant number of Sales Executives (Sales) and Research Scientists (R&D) earn between 150K and 200K.
    -   The Sales Manager role has the largest average compensation, followed by the HR Manager for the manager role.
    -   The Vice President role has the smallest average compensation.
...
-   Compensation by Experience levels
    ...
    -   Senior level of experience in the Sales department has the largest average compensation.
    -   The Expert level of experience in the HR department has the smallest average.
...
-   Compensation By Generations
    ...
    -   The Baby Boomers group from the HR has the smallest average compensation, followed by the Generation Z group from the Sales department.

-   Compensation By Gender

    -   There is a gender disparity in the number of employees across different compensation levels in the company.
    -   Women from the HR department have the smallest average (median) compensation compared with men within the same department and women from the other departments.
