...
-   A fictional company provides a range of products and services while sustaining a diverse workforce across various roles. However, the company's compensation strategy has remained unchanged for the past three years.
...
dept_job_title_comp_n <- employees1|>
...
ggplot(dept_job_title_comp_n, aes(x= annual_interval, y= job_title, fill = Count)) +
  geom_tile() + 
  labs(title = 'A significant number of Sales Executives (Sales) and Research Scientists (R&D) \n earn between 150.0K and 200.0K.',  
       x= 'Compensation', 
       y= 'Role') +
  guides(fill = guide_legend(title = "Number of Employees")) +
  scale_fill_continuous(low = "#E5E6E8FF", high = "#0070C0") +
...
dept_job_level_comp_n <- employees1|>
  group_by(dept, job_level, annual_interval)|>
  summarise(Count= n())

ggplot(dept_job_level_comp_n,aes(x= annual_interval, y= job_level, fill = Count)) +
  geom_tile() + 
  labs(title = 'A significant number of employees with intermediate levels of experience \nearn between 150.0K and 200.0K.',  
       x= 'Compensation', 
       y= 'Experience') +
...
ggplot(comp_gender_dept,aes(x= annual_interval, y= gender, fill = Count)) +
  geom_tile() + 
  labs(title = 'There is a balance between the portion of men and women \nwho earn between 150.0K and 200.0k.',  
       x= 'Compensation', 
       y= 'Gender') +
  guides(fill = guide_legend(title = "Employee Count")) +
  scale_fill_continuous(low = "#E5E6E8FF", high = "#0070C0") +
