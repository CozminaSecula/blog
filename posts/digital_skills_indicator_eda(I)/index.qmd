...
description: "An analysis of the components of the Digital Skills Indicator using data from the EU surveys on ICT (Information and Communication Technology) use in households and by individuals provided by Eurostat."
...
This post examines the composite indicators of the EU Digital Decade, with a focus on the Digital Skills Indicator (DSI). Previously, I explored the [Digital Intensity Index](https://cozminasecula.github.io/blog/posts/digital_intensity_index_components_eu/), which measures the digital transformation of businesses. This time, the focus is on the DSI, which assess the human capital dimension of the EU Digital Economy and Society Index (DESI).

The goal of this analysis is to improve our understanding of the current state of digital competencies within the EU population and to evaluate the progress made in 2023 compared to 2021. First, I will provide an overview of composite indicators and introduce the DSI. Then, using open data from Eurostat, I will perform an exploratory analysis of the DSI components and conclude by sharing key insights from the analysis.

## Introducing Digital Skills Indicator

In this section, I introduce the Digital Skills Indicator (DSI), a composite measure used to assess the digital competencies of individuals across the EU.

Before diving into composite indicators, it is essential to understand what indicators are. **Indicators** are tools used across various fields, such as economics, education, health, and environmental science, to monitor changes, trends, or conditions. You might already be familiar with key performance indicators (KPIs), which are quantitative or qualitative measures used to track progress toward specific goals. However, when dealing with complex, multi-dimensional issues, individual indicators may not suffice. This is where composite indicators come into play.
...
The Digital Skills Indicator (DSI) is crucial for tracking the EU’s progress toward its Digital Decade goals, specifically the target that by 2030, at least 80% of citizens aged 16-74 will possess at least basic digital skills.
...
The DSI focuses on people aged 16-74[^1], and for detailed information on DSI levels and the activities related to each skill, Eurostat provides comprehensive [metadata](https://ec.europa.eu/eurostat/cache/metadata/en/isoc_sk_dskl_i21_esmsip2.htm).

[^1]: Source: [Digital Economy and Society. Information on data](https://ec.europa.eu/eurostat/web/digital-economy-and-society/information-data#Information%20on%20variables,%20countries,%20and%20data%20collection)

## Competence Areas[^2]

[^2]: Source: [DigComp Framework](https://joint-research-centre.ec.europa.eu/oldpage-digcomp/digcomp-framework_en)
...
In today’s increasingly digital world, possessing digital skills is a necessity. Understanding DSI data is crucial as it provides insights into the current state of digital competencies within the EU population. This data is vital for policymakers, educators, businesses, and individuals to identify areas where digital skills are lacking, target interventions, and monitor progress toward digital literacy goals. Staying informed about digital skills data ensures that societies can effectively participate in the digital economy, foster innovation, and close the digital divide.

## Exploratory Data Analysis

The Exploratory Data Analysis (EDA) focuses on understanding DSI data, uncovering patterns, and generating hypotheses for further research. This involves visualizing data, calculating summary statistics, and identifying trends and relationships among variables.
...
The data analyzed in this post is sourced from Eurostat’s database on individuals’ digital skills. The analysis uses several packages, including `eurostat`[^3] for data retrieval, `tidyverse`[^4] for data manipulation and visualization, `janitor`[^5] for data cleaning, `reactable`[^6] for interactive tables and `patchwork`[^7] for plot composition.

[^3]: Lahti L., Huovari J., Kainu M., and Biecek P. (2017). Retrieval and analysis of Eurostat open data with the eurostat package. The R Journal 9(1), pp. 385-392. doi: 10.32614/RJ-2017-019 Lahti, L., Huovari J., Kainu M., Biecek P., Hernangomez D., Antal D., and Kantanen P. (2023). eurostat: Tools for Eurostat Open Data \[Computer software\]. R package version 4.0.0. https://github.com/rOpenGov/eurostat

[^4]: Wickham H, Averick M, Bryan J, Chang W, McGowan LD, François R, Grolemund G, Hayes A, Henry L, Hester J, Kuhn M, Pedersen TL, Miller E, Bache SM, Müller K, Ooms J, Robinson D, Seidel DP, Spinu V, Takahashi K, Vaughan D, Wilke C, Woo K, Yutani H (2019). “Welcome to the tidyverse.” *Journal of Open Source Software*, *4*(43), 1686. doi:10.21105/joss.01686 <https://doi.org/10.21105/joss.01686>.

[^5]: Firke S (2023). *janitor: Simple Tools for Examining and Cleaning Dirty Data*. R package version 2.2.0, <https://CRAN.R-project.org/package=janitor>.

[^6]: Lin G (2023). *reactable: Interactive Data Tables for R*. R package version 0.4.4, <https://CRAN.R-project.org/package=reactable>.

[^7]: Pedersen T (2024). *patchwork: The Composer of Plots*. https://patchwork.data-imaginist.com, https://github.com/thomasp85/patchwork.
...
In this data there are 397.473 observations recorded, each having the following properties:

1.  `freq` - the frequency of data collection
2.  `ind_type` - individual type
3.  `ind_is` - information society indicator
4.  `unit` - unit of measure the value is expressed on
5.  `geo` - geopolitical entity, country
6.  `TIME PERIOD` - time, the year the survey was carried on
7.  `values` - values

There are 54 individual type with missing values. I will keep them as they are, as the data may not exist.

For more detailed information about the dataset, refer to [metadata on individuals' level of digital skills](https://ec.europa.eu/eurostat/cache/metadata/en/isoc_sk_dskl_i21_esmsip2.htm).
...
reactable(digital_skills_summary,
          ...
          columnGroups = list(
            colGroup("Indicator", columns = c("digital_skills", "indicator_level"), sticky = "left"),
            colGroup("Year", columns = "year", sticky = "left"),
...
plot_skill_histograms <- function(data, skill) {
  ...
  plot_basic <- ggplot(skill_data, aes(x = individuals_with_basic_skills)) +
    geom_histogram(color = "#252525",
                   fill = colors["Basic Skills"],
                   binwidth = function(x) (max(x) - min(x)) / 5) +
    geom_vline(aes(xintercept = mean(individuals_with_basic_skills,
                   na.rm = TRUE)), 
                   color = "#252525", 
                   linetype = "dashed",
                   linewidth = 1) +
  ...
  plot_above_basic <- ggplot(skill_data, aes(x = individuals_with_above_basic_skills)) +
    geom_histogram(color = "#33645FFF", 
                   ...
                   binwidth = function(x) (max(x) - min(x)) / 5) +
    geom_vline(aes(xintercept = mean(individuals_with_above_basic_skills,                       na.rm = TRUE)), 
                   color = "#33645FFF", 
                   linetype = "dashed", 
                   linewidth = 1) +
    facet_grid(. ~ year) +
  ...
  plot_basic_or_above <- ggplot(skill_data, aes(x = individuals_with_basic_or_above_basic_skills)) +
    geom_histogram(color = "#9D1B1FFF",
                   ...
                   binwidth = function(x) (max(x) - min(x)) / 5) +
    geom_vline(aes(xintercept =   mean(individuals_with_basic_or_above_basic_skills, 
                   na.rm = TRUE)), 
                   color = "#9D1B1FFF", 
                   linetype = "dashed", 
                   linewidth = 1) +
...
legend_plot <- ggplot() +
  ...
  annotate("text", x = 0.15, y = 0.98, label = "Basic Skills", size = 4, hjust = 0, color = "#252525") +
  ...
  annotate("text", x = 0.08, y = 0.9, label = "2021", size = 4, hjust = 0, color = "#252525") +
  annotate("text", x = 0.48, y = 0.9, label = "2023", size = 4, hjust = 0, color = "#252525") +
...
::: grid
::: g-col-6
![Histogram[^8]](dsi_eu_histogram.png)
:::

::: g-col-6
![Boxplot[^9]](dsi_eu_boxplot.png)
:::
:::
:::

[^8]: [Histogram](https://flowingdata.com/wp-content/themes/fd-modern-6-0/images/chart-types/histogram.png) ![](images/clipboard-249601241.png)


[^9]: [Reading a Box-and-Whisker Plot](https://i0.wp.com/flowingdata.com/wp-content/uploads/2008/02/box-plot-explained.gif?w=1090) ![](images/clipboard-3227116470.png)


## Trends in Digital Skills Across Categories, Years, and Skill Levels

Below are some key observations beased on the above analysis:
...
4.  **At Least Basic Skills**

The distinction between basic or above basic (at least basic skills) and "above basic" skill levels shows that a significant portion of individuals meet at least the basic criteria, but fewer achieve above basic proficiency. For example, in 2023, 57.63% of individuals had basic or above basic *Overall Digital skills*, but only 28.98% reached above basic proficiency. This highlights the need for policies aimed at helping more individuals advance to above basic level. Such progress is crucial for the EU to meet its ambitious digital skills goals.

5.  **Safety and Digital Content Creation Skills**
